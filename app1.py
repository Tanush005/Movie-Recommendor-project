...
@st.cache_resource
def load_data():
    # 🔽 Download similarity.pkl from Google Drive (only once due to caching)
    ...
    # ✅ Load pickled files
    similarity = pickle.load(open('similarity.pkl', 'rb'))
    movies = pickle.load(open('movies.pkl', 'rb'))     # Already local
    movie1 = pickle.load(open('movie1.pkl', 'rb'))     # Already local
...
if recommend_button:
    ...
    # ✅ Show latency in Streamlit
    st.success(f"✅ Recommendations generated in {latency_ms:.2f} ms")
...
<hr style="border:1px solid gray; margin-top: 50px;">
<p style="text-align:center; color: #888;"> Built with ❤️ by Tanush </p>
>>>>>>> e33d916 (Initial commit for Streamlit Movie Recommender)
""", unsafe_allow_html=True)